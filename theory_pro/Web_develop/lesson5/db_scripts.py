...
def get_quiz_questions(quiz_id):
    open()
    query = """
    SELECT quiz_content.id, question.question
...
def get_question_after(quiz_id, last_question):
    open()
    query = """
    ...
    ORDER BY quiz_content.id
    """
    cursor.execute(query, [quiz_id, last_question])
    result = cursor.fetchone()
    close()
    return result
...
def main():
    quiz_id = input("Какой номер викторины?")
    last_question = input("Какой последний вопрос?")
    print(get_question_after(quiz_id, last_question))
